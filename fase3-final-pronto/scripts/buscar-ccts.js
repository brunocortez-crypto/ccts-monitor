...
// Encontrar pasta data — tenta diferentes caminhos
function encontrarData() {
  const opcoes = [
    path.join(__dirname, '..', 'data'),
    path.join(__dirname, '..', '..', 'data'),
    path.join(process.cwd(), 'data'),
    path.join(process.cwd(), 'fase3', 'data')
  ];
  
  for (const dir of opcoes) {
    if (fs.existsSync(dir)) {
      console.log(`✅ Pasta data encontrada: ${dir}`);
      return dir;
    }
  }
  
  throw new Error(`❌ Pasta 'data' não encontrada em nenhum lugar!`);
}

const DATA_DIR = encontrarData();
...
function carregar(arq, padrao) {
  try { 
    console.log(`📖 Carregando: ${arq}`);
    const conteudo = JSON.parse(fs.readFileSync(arq, 'utf8')); 
    console.log(`✅ Arquivo carregado!`);
    return conteudo;
  } catch (e) { 
    console.log(`⚠️ Erro ao carregar ${arq}: ${e.message}`);
    return padrao; 
  }
}
...
function salvar(arq, dados) {
  try {
    ...
    console.log(`❌ Erro ao salvar ${arq}: ${e.message}`);
  }
}
